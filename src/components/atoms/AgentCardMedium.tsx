...
export default function AgentCardMedium({
  id,
  rank,
  portrait,
  name,
    ...
    <article className="flex flex-col  max-w-90 bg-zinc-800  border-2 border-fosfo-500 rounded-2xl ">
      ...
      <section className="flex flex-col p-4 gap-2">
        <div className="flex flex-col gap-3">
          <h2 className="p-2 text-2xl text-center font-semibold font-titles bg-zinc-900 rounded-2xl text-fosfo-500">
            {name}
          </h2>
          <p className="bg-zinc-900 rounded-2xl text-fosfo-500 font-titles text-center text-fosfo-400 tracking-wide uppercase">
            {faction}
          </p>
        </div>
      ...
      <table className="font-titles border-b-2  border-fosfo-500 text-white w-full">
        <thead className="border-b-2 border-b-fosfo-500">
          <tr className="text-center">
            <th className=" border-r-2 border-r-fosfo-500   text-sm">{attribute}</th>
            <th className=" p-2 text-sm">{specialty}</th>
            <th className="border-l-2 border-l-fosfo-500 p-2 text-sm">{type}</th>
          </tr>
        </thead>
      ...
      <section className="text-white flex flex-col p-2.5 gap-1">
        {agent_intel.split("\\n").map((linea, index) => (
          <span key={index}>
            <p>{linea}</p>
          </span>
